...
const preset = extractPreset(JSON.stringify({ extends: ['github>edbfi/automation//default.json#v1.0.0'] }), 'renovate.json');
assert.equal(preset.deps[0].currentValue, 'v1.0.0');
assert.equal(preset.deps[0].depName, 'edbfi/automation');
const actions = await extractActions('jobs:\n  check:\n    uses: edbfi/automation/.github/workflows/bun.yml@v1.0.0\n', '.github/workflows/ci.yml');
assert.equal(actions.deps[0].currentValue, 'v1.0.0');
assert.equal(actions.deps[0].depName, 'edbfi/automation');
